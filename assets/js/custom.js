...
// fetch siteplug recommended apps
const fetchRecommends = async () => {
    let count = '8';
    let imgRes = '128x128';
    await fetch(`https://kme46.siteplug.com/qlapi?o=kme46&s=88107&f=json&n=${count}&i=1&is=${imgRes}`)
        .then(res => res.json())
    ...
    function renderItems(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            let single_item = `<div class="panel-body">
                    <a id="app_url_${i}" href="${data[i]['rurl']}"></a>
                    ...
                    <span class="panel-heading word-wrap recommended_apps_name"
                        id="recommended_apps_name_${i}">${data[i]['brand']}</span>
                </div>`;
...
window.addEventListener('load', function () {
    window.addEventListener('scroll', handleScrollDesktop); //desktop
    dqs('body').addEventListener('touchmove', handleScrollMobile); //mobile
});

function handleScrollDesktop() {
    if ((docHeight - winHeight < (window.pageYOffset + 50)) && (window.pageYOffset + 50 >= lastBottomPos)) {
        if (!alreadyLoading) {
            console.log('%c Called!, %c(docHeight - winHeight): ' + (docHeight - winHeight) + '%c pageYOffset: ' + pageYOffset, 'background: #aaa; color: green', 'background: #aaa; color: red', 'background: #aaa; color: purple');
            alreadyLoading = true;
            lastBottomPos = window.pageYOffset + 50;
            loadMoreNews();
        }
    }
}
...
function handleScrollMobile() {
    if ((docHeight - winHeight < (window.pageYOffset + 50)) && (window.pageYOffset + 50 >= lastBottomPos)) {
        if (!alreadyLoading) {
            console.log('%c Called!', 'background: #aaa; color: green');
            alreadyLoading = true;
            lastBottomPos = window.pageYOffset + 50;
            loadMoreNews();
        }
...
function loadMoreNews() {
    worker.postMessage({ event: 'load_more_news', current_parameter: current_parameter });
}
